...
def screen2_redrawAll(app):
    ...
    if app.maker or app.saveboard!=None:
        drawLabel('Custom Sudoku', app.width//2, app.height//10, size=60,fill = 'black')
    else:
        drawLabel(f'Sudoku ({app.difficulties[app.selectedDiff]})', app.width//2, app.height//10, size=60,fill = 'black')
    drawBoard(app)
    drawBoardBorder(app)
    if not app.maker:
        drawFlags(app)
    ...
    if app.maker:
        drawNumbers(app)
        drawRect(1111,650,351,100,fill = 'lightblue',border = 'black')
        drawLabel("Make Board",1286.5,697.5,align = 'center',size =40)
        drawRect(50,30,300,60,fill = 'lightblue',border = 'black')
        drawLabel("Back to Menu",200,60,size = 40,bold = True,align = 'center')
    elif (app.selectedMode!= None and (app.modes[app.selectedMode] == 'mouse-only' or app.modes[app.selectedMode] == 'standard')):
        drawNumbers(app)
    ...
    if app.showPossibles == True:
        drawPossibles(app)
    if sudokuSolved(app):
        drawRect(app.width//2,app.height-50,400,100,align = 'center',fill = 'white',border = 'black')
...
def drawPossiblesControl(app):
    drawRect(50,app.numpadStartTop,app.width - app.numpadStartLeft-50,app.boardHeight//4*3,fill = 'cyan', border = 'black')
    for i in range(5):
        drawRect(225,200+i*80,(app.width - app.numpadStartLeft-50)//1.5,40,align = 'center',fill= None,border = 'black')
        drawLabel(app.listofActions[i],225,200+i*80,size = 20,bold = True)
...
def drawNumbers(app): #Modified from CS Academy
    drawRect(app.numpadStartLeft,app.numpadStartTop,app.width - app.numpadStartLeft-50,app.boardHeight//4*3,fill = 'cyan', border = 'black')
    numpadWidth = app.width - app.numpadStartLeft - 50
    numpadHeight = app.boardHeight//4*3
    for i in range(3):
        for j in range(3):
            drawNumCell(app,i,j,app.numpad[i][j])
    drawRect(1151,500,80,40,fill = None,border = 'black')
    drawRect((1111+1462)//2,520,80,40,align = 'center',fill = None,border = 'black')
    drawRect(1340,500,80,40,fill = None, border = 'black')
    drawLabel("Undo",1191,520,size = 20)
    drawLabel("Redo",(1111+1462)//2,520,size = 20)
    drawLabel("Clear",1380,520,size = 20)#numpadStartLeft+ (numpadWidth-200 - width)//2 + i*100, numpadStartTop+30 + j*100, width,height,fill = 'lightgrey',border = 'black')
def drawNumCell(app, row, col,number): #Modified from CS Academy
    ...
    drawLabel(app.numpad[row][col],cellLeft+cellWidth//2,cellTop+cellHeight//2, 
              align = 'center',size = 32,fill = 'black')
...
def drawBoard(app): #From CS Academy
    ...
    for i in range(1,3):
        drawLine(app.boardLeft + i * app.boardWidth//3, app.boardTop,app.boardLeft + i * app.boardWidth//3, app.boardTop+app.boardHeight, lineWidth = app.cellBorderWidth*2,fill='black')
    for j in range(1,3):
        drawLine(app.boardLeft,app.boardTop + j*app.boardHeight//3,app.boardLeft+app.boardWidth,app.boardTop+j*app.boardHeight//3, lineWidth = app.cellBorderWidth*2,fill = 'black')
def drawBoardBorder(app): #From CS Academy
  # draw the board outline (with double-thickness):
    drawRect(app.boardLeft, app.boardTop, app.boardWidth, app.boardHeight, border='black',
...
def screen2_onMouseRelease(app,mouseX,mouseY):
        ...
        if 30<=mouseY<=90:
            ...
            if selectedCell!=None:
                ...
                if 500<= mouseY<=540:
                    ...
                    app.message = ''
                    setActiveScreen('screen1')
        if app.board!= [] and not app.maker:
            updatePossibles(app)
            updateStates(app)


def screen2_onMouseMove(app,mouseX,mouseY):
    if app.maker or (app.selectedMode!= None and (app.modes[app.selectedMode] == 'mouse-only' or app.modes[app.selectedMode] == 'standard')):
